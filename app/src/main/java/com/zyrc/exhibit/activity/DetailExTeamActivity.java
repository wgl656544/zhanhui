...
public class DetailExTeamActivity extends BaseActivity implements View.OnClickListener {
    @ViewInject(R.id.iv_detail_ex_team_back)
    private ImageView ivBack;
    @ViewInject(R.id.lv_detail_ex_team)
    ...
    private Handler handler = new Handler() {
        ...
        public void handleMessage(Message msg) {
            ...
            if (msg.what == HandlerConstant.SEARCH_SUCCESS) {
                CommonBean detailExTicketBean = (CommonBean) msg.obj;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_detail_ex_team);
        x.view().inject(this);
        mActivity = this;
        setListeners();
        int exhibId = getIntent().getIntExtra("entityId", 1);
        model.search(handler, UrlConstant.HTTP_URL_DETAIL_EX_TEAM, "?exhibId=" + exhibId);
        startLoading("加载中...");
    }
    ...
    public void onClick(View v) {
        switch (v.getId()) {
            case R.id.iv_detail_ex_team_back:
                finish();
                break;
        }
    }
    ...
    private void showTicket(CommonBean bean) {
        ...
        TicketAdapter adapter = new TicketAdapter(this, datas);
